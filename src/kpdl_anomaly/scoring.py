...
from .schema import CELL_SCORE_COLUMNS, FRAME_SCORE_COLUMNS, METADATA_COLUMNS, SCHEMA_VERSION
...
def score_features(
    ...
    with config.test_feature_path.open("r", newline="", encoding="utf-8") as input_handle, cell_scores_path.open(
        ...
        for row in reader:
            ...
            cell_score = (
                config.cluster_weight * score["cluster_distance_score"]
                + config.temporal_weight * temporal_change_score
            )
